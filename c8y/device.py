...
class C8yDevice():
    ...
    def send_measurement(self, data):
        """
        Sends a measurement, with argument as body using HTTP POST. Returns True if result was code 201, False otherwise
        :param data: the measurement body (dict)
        """
        ...
        try:
            ...
            print("[e] Error Sending Measurement: ", e)
            return False
    ...
    def send_event(self, data):
        """
        Send event using HTTP POST. Returns True if event was created, False otherwise
        :param data: the event body (dict)
        """
        request_url = self.cumu_url + "/event/events"
        headers = c8y_http.HEADER_GENERIC_JSON

        try:
            resp = requests.post(request_url, auth=self.auth, data=json.dumps(data), headers=headers)
        except ConnectionError as e:
            print("[e] Error Sending Event: ", e)
            return False

        if resp.status_code == 201:
            print("[i] Sent Event: ", resp.text)
            return True
        else:
            print("[e] Unable to send event: ", resp.status_code)
            print(resp.text)
            return False

    def spawn_child_device(self, name, supported_measurements):
        """ Create Child Device and link it to this device.
             Returns child ID if successfully created"""
